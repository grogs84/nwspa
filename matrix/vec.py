...
def equal(u,v):
    ...
    assert u.D == v.D
    return (sum([v[i] == u[i] for i in u.D]) == len(u.D))

def add(u,v):
    "Returns the sum of the two vectors"
    assert u.D == v.D
    return Vec(set(u.D), {i:u[i] + v[i] for i in u.D})
...
def scalar_mul(v, alpha):
    ...
    return Vec(set(v.D), {i:alpha*v[i] for i in v.D})

def neg(v):
    "Returns the negation of a vector"
    return Vec(set(v.D), {i:-1.0 * v[i] for i in v.D})
