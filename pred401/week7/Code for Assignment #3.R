...
n <- 400  #  This is the sample size which will be changed repeatedly for this part.
...
t.test(births, conf.level = 0.95, alternative = 'greater')
t.test(births, conf.level = 0.99, alternative = 'greater')

# Determine 95% and 99% one-sided confidence intervals using percentile bootstrap method.
round(quantile(births.mean, probs=0.05, digits = 2))
round(quantile(births.mean, probs=0.01, digits = 2))

# Determine 95% and 99% one-sided confidence intervals using bootstrap t-test.
# Determine two-sided bootstrap t confidence intervals.
Q2 <- quantile(births.t, prob=c(0.95), names = FALSE)
round(mu -Q2*sd(births)/sqrt(n), digits = 2)


Q2 <- quantile(births.t, prob=c(0.99), names = FALSE)
round(mu -Q2*sd(births)/sqrt(n), digits = 2)
...
t.test(personnel, conf.level = 0.90, alternative = 'less')
...
Q2 <- quantile(personnel.t, prob=c(0.90), names = FALSE)
round(mu -Q2*sd(personnel)/sqrt(n), digits = 2)
