...
print 'volume is: {}'.format(round(volume,1))
...
def shipping(fixed, unit_cost, units):
	return fixed + unit_cost*(units-1)


def wholesale_cost(units, cover_price, discount):
	return units*((1-discount)*cover_price)
...
wholesale_cost  = wholesale_cost(units, cover_price, discount) + shipping(fixed, unit_cost, units)

print "wholesale cost is: {}".format(round(wholesale_cost,2))
print '\n'
...
start = 6*60+52 # start time in minutes

easy_pace = 8+(15/60.0) # 8min + 15/60.0 = 8min 15 sec = 8.25

fast_pace = 7+(12/60.0)

time = 2*easy_pace + 3*fast_pace
