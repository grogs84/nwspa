...
#
# This works, but not great. Projects tend to spend more less and more slowly during design, then 
# as construction starts increases quickly, and finally drops to almost zero during closeout. If the
# costs as spread evenly in the budget it will seem as though projects are in design, blowing the budget in
# construction and under spending again in closeout. 
#
# One way to create a more accurate monthly estimate for project is to model it after completed projects 
# monthly spend. To do this we fit a polynomial function to a series of points (x, y) using excel where:
#
# x = % complete a project is (month of spend/# of months)
# y = % of project spend (monthly spend / total project cost)
#
#
...
# months is sometimes small this sum will be less than the buget. As the number of months approces infinity the sum
...
ys = np.array([(Fg(x+1./months)*budget)-(Fg(x)*budget) for x in xs])
